...
export default function UserAppoinment() {
  ...
  const idFilter = (appointments, filter) => {
    return appointments.filter((appointment) => appointment.creator_id.includes(filter))
  }
    ...
    <>
      <section className="w-full bg-hijau-muda h-[100vh] flex justify-center ">
        <div className="px-[100px] overflow-x-scroll w-full mt-[70px]">
          ...
          <AppointmentConfig appointments={statusFilter(appointments, filter)} />
        </div>
      </section>
    </>
  );
      ...
      {appointments.map((appointment) => {
        return (
          <div key={appointment.key} className="grid grid-cols-5 bg-white">
            ...
            <p className="text-center py-[26px]">{appointment.receiver_name.name}</p>
